...
hbs.registerHelper('chiefwarden', function(lvalue, options) {
	...
	hostel.getHostelById(hostelId,(err,queryRep)=>
	{
		...
		utility.getNotifications(hostelId, 5, (error, topNotifications) =>{
				...
				users.getUserByHostelId(hostelId,(error,hostelUser)=>{
					...
					var wardenList = [];
					   wardens.getWardenByHostelId(hostelId,(error,warden)=>{

                             if(error){
								 console.log("error in getwardenbyid");
								 response.send(error);
							 }
							 else
							 {
								 ...
								 {
								 warden.forEach((warden)=>{
									   wardenList.push({name:warden.name,contact:warden.contact});
								 })
								}
							 }
