...
function setUpRoutes(app){
  ...
  //create wardens by satyendra


  app.post('/create_warden',
  require('connect-ensure-login').ensureLoggedIn(loginRequired),
  function(req, res){
    if(req.user.is_admin==0){
      res.send("You are not the admin, you cannot create another user");
    ...
    warden.createNewWarden(wardenDetails,(err,created)=>
    {
      if(!err&&created==true)
      {
        res.redirect('/Manage');
  ...
  function(req, res){
    ...
    student.addStudents(studentDetails,(err,created)=>
    {
      if(!err&&created==true)
      {
        res.redirect('/Manage');
    ...
    hostels.getHostelById(req.user.hostel_id, (err, hostelDetails) => {
      mess_menu.getMessMenu(req.user.hostel_id, (error, result) => {
          ...
          users.getAllUsers((error,result)=>{
              if(error)
                {
                  console.log("error in getmember by id");
                }
                if(result)
                {
                  var userList = [];
          ...
          console.log("isadmin and hid " + result[i].is_admin + " " + req.user.hostel_id + " " + result[i].hostel_id)            
          if(result[i].is_admin==2 && result[i].hostel_id==req.user.hostel_id)
          {
            // hostelNames.forEach((x) => {
                // if(x.hid == result[i].hostel_id) {
                  userList.push({
  ...
  app.post("/EditProfile",require('connect-ensure-login').ensureLoggedIn(loginRequired),(req,res)=>
  {
    ...
    var image=req.files.image;
    if(!fs.existsSync(__dirname+'/public/images/users/'))
    {
      fs.mkdirSync(__dirname+'/public/images/users/');
    }
  ...
  app.post('/complain',(req,res)=>{
           ...
           var cno = req.body.contactno;
           var email = req.body.email;
           var course = req.body.course;
           var subject = req.body.subject;

           console.log("complain .. "+name + " " + reg + " " + cno + " " + email + " " + course + " " + subject);
           res.redirect('/');
